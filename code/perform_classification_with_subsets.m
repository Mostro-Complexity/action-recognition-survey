function [] = perform_classification_with_subsets(root_dir, subject_labels,...
    ...
    for set = 1:n_action_sets
        ...
        for i = 1:length(confusion_matrices)
            xlswrite([root_dir, '/confusion_matrices_as', num2str(set),'.xlsx'], confusion_matrices{i},...
                ['confusion_matrix', num2str(i)])
        end
        xlswrite([root_dir, '/avg_confusion_matrix_as', num2str(set),'.xlsx'], avg_confusion_matrix)
    end
